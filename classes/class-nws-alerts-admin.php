...
class NWS_Alerts_Admin {
    ...
    */
    ...
    public static function add_settings_page() {
        $alerts_bar_controls = array();
        $google_maps_controls = array();
        ...
        if (!empty($_POST)) {
            if (isset($_POST['nws_alerts_alerts_bar_action']) && $_POST['nws_alerts_alerts_bar_action'] === 'update' && check_admin_referer('update', 'nws_alerts_alerts_bar_nonce')) {
                ...
                $control = 'error';
                $key = $prefix . $control;
                $alerts_bar_controls[$control] = false;

                $control = 'zip';
                $key = $prefix . $control;
                if (isset($_POST[$key])) {
                    update_option($key, $_POST[$key]);
                    $alerts_bar_controls[$control] = $_POST[$key];
                }
                ...
                if (isset($_POST[$key]) && $_POST[$key] == 'on') {
                    update_option($key, 1);
                    $alerts_bar_controls[$control] = true;
                } else {
                    update_option($key, 0);
                    $alerts_bar_controls[$control] = false;
                }
                ...
                if (isset($alerts_bar_controls['zip']) && !empty($alerts_bar_controls['zip'])) $allow_enabled = true;
                if (isset($alerts_bar_controls['city']) && !empty($alerts_bar_controls['city']) && isset($alerts_bar_controls['state']) && !empty($alerts_bar_controls['state'])) $allow_enabled = true;
                if (isset($alerts_bar_controls['state']) && !empty($alerts_bar_controls['state']) && isset($alerts_bar_controls['county']) && !empty($alerts_bar_controls['county'])) $allow_enabled = true;
                ...
                if ($allow_enabled && isset($_POST[$key]) && $_POST[$key] == 'on') {
                    ...
                    if (isset($_POST[$key]) && $_POST[$key] == 'on') {
                        $alerts_bar_controls['error'] = 'Not enough location information was provided to enable the Alerts Bar';
                    }
            ...
            if (isset($_POST['nws_alerts_google_maps_action']) && $_POST['nws_alerts_google_maps_action'] === 'update' && check_admin_referer('update', 'nws_alerts_google_maps_nonce')) {
                $prefix = 'nws_alerts_google_maps_';

                $control = 'api_key';
                $key = $prefix . $control;
                if (isset($_POST[$key])) {
                    update_option($key, $_POST[$key]);
                    $google_maps_controls[$control] = $_POST[$key];
                }
            }
        }
        ...
        echo self::get_module('build-tables', array());
        echo self::get_module('google-maps', $google_maps_controls);
        echo self::get_module('alerts-bar', $alerts_bar_controls);
    ...
    */
    public static function get_module($module = '', $controls = array()) {
        if ($module === 'alerts-bar') {
            ...
            $return_value .= '</div>';
            $return_value .= '</div>';
        } else if ($module === 'google-maps') {
            $defaults = array('api_key' => NWS_ALERTS_GOOGLE_MAPS_API_KEY);
            $controls = wp_parse_args($controls, $defaults);
            $return_value = '';
            $module_id_prefix = 'nws-alerts';
            $control_id_prefix = $module_id_prefix . '-' . $module;

            $return_value .= '<div class="metabox-holder"><div class="meta-box-sortables ui-sortable"><div class="postbox"><h3 class="hndle"><span>Google Maps</span></h3>';
            $return_value .= '<div class="inside">';
                $return_value .= '<p class="description">The Google Maps JavaScript API now requires an API key in order to display maps on websites. Due to the map API quota limits this API key cannot be included with the plugin and a unique key must be obtained by you.</p>';
                $return_value .= '<p class="description">You can obtain a Google Maps JavaScript API key by visiting the <a href="https://developers.google.com/maps/documentation/javascript/get-api-key" target="_blank">Get a Key/Authentication</a> page and following the instructions. Once a project has been created and an associated key has been generated, simply copy it and paste it into the "Google Maps API Key" field below.</p>';
                $return_value .= '<form id="' . $control_id_prefix . '" method="post" action="">';
    ...
    */
    public static function get_control($control, $control_id_prefix, $default = false) {
        ...
        if ($control === 'action') {
            ...
            $return_value .= '<tr>';
				$return_value .= '<td><h4>Google Maps API Key</h4></td>';
				$return_value .= '<td>';
